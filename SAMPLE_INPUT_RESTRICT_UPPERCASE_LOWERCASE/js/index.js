	...
	function initialize($, Debug, FormUtil) {
		...
		function init() {
			...
			*/
			...
			/* restrict uppercase english setting */
			var _$input_2 = $(".registration_1");
			var _input_2 = _$input_2.get(0);
			_input_2.style.imeMode = "disabled";
			_input_2.style.textTransform = "uppercase";
			_$input_2.bind("keyup", uppercaseInputKeyHandler);
			
			/* restrict lowercase english setting */
			var _$input_2 = $(".registration_2");
			var _input_2 = _$input_2.get(0);
			_input_2.style.imeMode = "disabled";
			_input_2.style.textTransform = "lowercase";
			_$input_2.bind("keyup", lowercaseInputKeyHandler);
		}
		...
		function uppercaseInputKeyHandler($evt) {
			switch($evt.type) {
				case "keyup" :
					if($evt.which) {
						if($evt.which > 36 && $evt.which < 41 || $evt.which == 46 || $evt.which == 8) return; //left, top, right, down, delete, backspace
						//if($evt.which > 47 && $evt.which < 58) return; //restrict 0 ~ 9
						if($evt.which > 47 && $evt.which < 58) { 
							setInputRestrictNumber($evt.currentTarget)
							return;
						}
						setInputCustomImeModeDisabled($evt.currentTarget);
					}
		...
		function lowercaseInputKeyHandler($evt) {
			switch($evt.type) {
				case "keyup" :
					if($evt.which) {
						if($evt.which > 36 && $evt.which < 41 || $evt.which == 46 || $evt.which == 8) return; //left, top, right, down, delete, backspace
